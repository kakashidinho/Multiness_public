...
namespace Nes
{
	namespace Core
	{
		...
		public:
			...
			typedef std::function<void(Sound::Output&)> PostprocessCallback;
			//end LHQ
		...
		private:
			...
			Sound::Output* stream;
			FrameOutput frameSnapshot;//LHQ
			bool frameSnapshotEnabled;//LHQ
			PostprocessCallback postprocessCallback;//LHQ
			Sound::Buffer buffer;
			Settings settings;
		...
		public:
			...
			//LHQ
			void EnableFrameSnapshot(bool e) {
				frameSnapshotEnabled = e;
			}

			const FrameOutput& GetFrameSnapshot() const {
				return frameSnapshot;
			}

			uint GetSampleBlockAlign() const {
				return frameSnapshot.blockAlign;
			}
