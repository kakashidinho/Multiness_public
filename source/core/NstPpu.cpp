...
namespace Nes
{
	namespace Core
	{
		...
		//LHQ
		void Ppu::ResetColorUseCountTable() {
			for (int i = 0; i < Video::Screen::PALETTE; ++i)
			{
				this->colorUseCountTable[i].idx = i;
				this->colorUseCountTable[i].count = 0;
			}
		}

		NST_FORCE_INLINE void Ppu::RenderPixelPostProcess(uint index, Video::Screen::Pixel value) {
			//LHQ: count number of pixels use a specific color
			if (this->colorUseCountShouldRestart)
			{
				//was requested to reset color use count table. perform it now
				ResetColorUseCountTable();

				this->colorUseCountShouldRestart = false;
			}
			this->colorUseCountTable[value].count++;
		}
