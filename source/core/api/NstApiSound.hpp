	...
	namespace Core
	{
		namespace Sound
		{
			...
			*/
			...
			public:
				...
				*/
				...
				typedef void (NST_CALLBACK *UpdateSettingsCallback) (void* userData);//LHQ
				...
				*/
				...
				static SettingsUpdater updateSettingsCallback;//LHQ
			};
			...
			*/
			...
			//LHQ
			struct Output::SettingsUpdater : UserCallback<Output::UpdateSettingsCallback>{
				void operator () () const
				{
					if (function)
						function(userdata);
			...
			class Input {
			public:
				typedef Output::UpdateSettingsCallback UpdateSettingsCallback;
				typedef Output::SettingsUpdater SettingsUpdater;
				struct Reader;

				//callback to read input sound data. pass <samples> = NULL to query how many samples are currently available for reading
				typedef uint (NST_CALLBACK *ReadCallback) (void* userData, Input& input, void* samples, uint maxSamples);

				static Reader readCallback;
				static SettingsUpdater updateSettingsCallback;
			};

			struct Input::Reader : UserCallback<Input::ReadCallback> {
				uint operator () (Input& input, void* samples, uint maxSamples) const
				{
					if (function)
						return function(userdata, input, samples, maxSamples);
	...
	namespace Api
	{
		...
		*/
		...
		public:
			...
			* Sound input context
			*/
			typedef Core::Sound::Input Input;
		};
	}
}
