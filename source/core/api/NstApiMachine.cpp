...
namespace Nes
{
	namespace Api
	{
		...
		Result Machine::LoadRemote(std::shared_ptr<HQRemote::IConnectionHandler> connHandler, const char* clientInfo) throw() {
			return emulator.LoadRemote(connHandler, clientInfo);
		}

		Result Machine::LoadRemote(const std::string& remoteIp, int remotePort, const char* clientInfo) throw() {
			return emulator.LoadRemote(remoteIp, remotePort, clientInfo);
		}

		Result Machine::EnableRemoteController(uint idx, std::shared_ptr<HQRemote::IConnectionHandler> connHandler, const char* hostInfo) throw()
		{
			return emulator.EnableRemoteController(idx, connHandler, hostInfo);
		}
		
		Result Machine::EnableRemoteController(uint idx, int networkListenPort, const char* hostInfo) throw() {
			return emulator.EnableRemoteController(idx, networkListenPort, hostInfo);
		}
		...
		{
			return emulator.RemoteControllerConnected(idx);
		}

		bool Machine::RemoteControllerEnabled(uint idx) const
		{
			return emulator.RemoteControllerEnabled(idx);
		}

		void Machine::EnableLowResRemoteControl(bool enable) {
			emulator.EnableLowResRemoteControl(enable);
		}

		Result Machine::SendMessageToRemote(uint64_t id, const char* message)
		{
			return emulator.SendMessageToRemote(id, message);
		}

		const char* Machine::GetRemoteName(uint remoteCtlIdx) const {
			return emulator.GetRemoteName(remoteCtlIdx);
		}
