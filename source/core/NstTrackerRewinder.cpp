...
namespace Nes
{
	namespace Core
	{
		...
		void Tracker::Rewinder::Execute(Video::Output* videoOut,Sound::Output* soundOut,Input::Controllers* inputOut,Sound::Input* inputSoundOut)
		{
			try
			{
				if (uturn)
				...
				else
				{
					...
					if (rewinding)
					{
						...
						(emulator.*emuExecute)( videoOut, soundOut, inputOut, inputSoundOut );
						return;
					}
				}
			}
		...
		void Tracker::Rewinder::ChangeDirection()
		{
			...
			if (rewinding)
			{
				for (uint i=frame; i < LAST_FRAME; ++i)
					(emulator.*emuExecute)( NULL, NULL, NULL, NULL );
...

				{
					...
					{
						video.Store();
						(emulator.*emuExecute)( NULL, sound.Store(), NULL, NULL);
					}
				}
