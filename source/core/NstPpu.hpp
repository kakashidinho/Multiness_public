...
namespace Nes
{
	namespace Core
	{
		...
		public:
			//LHQ
			struct ColorUseCount {
				Video::Screen::Pixel idx;
				uint count;
			};

			typedef ColorUseCount ColorUseCountTable[Video::Screen::PALETTE];
			//end LHQ
		...
		private:
			...
			NST_FORCE_INLINE void RenderPixelPostProcess(uint index, Video::Screen::Pixel value);//LHQ
		...
		public:
			...
			const ColorUseCountTable& GetColorUseCountTable() {
				return colorUseCountTable;
			}

			void ResetColorUseCountTable();
