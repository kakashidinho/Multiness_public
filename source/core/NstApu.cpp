...
namespace Nes
{
	namespace Core
	{
		...
		extChannel (NULL),
		buffer     (16),
		frameSnapshotEnabled(false),//LHQ
		postprocessCallback(nullptr)//LHQ
		{
		...
		void Apu::UpdateSettings()
		{
			...
			//LHQ
			frameSnapshot.blockAlign = settings.bits / 8;
			if (settings.stereo)
				frameSnapshot.blockAlign *= 2;
		}
		...
		void Apu::EndFrame()
		{
			...
			if (updater != &Apu::SyncOff)
			{
				...
				if (Sound::Output::lockCallback( *stream ))
				{
					...
					//LHQ: save original sound output's snapshot for current frame
					size_t regionSizeBytes[2];
					regionSizeBytes[0] = stream->length[0] * GetSampleBlockAlign();
					regionSizeBytes[1] = stream->length[1] * GetSampleBlockAlign();

					if (frameSnapshotEnabled)
					{
						frameSnapshot.samples.clear();
						for (int i = 0; i < 2; ++i)
						{
							if (stream->samples[i])
							{
								auto samplesSizeInBytes = regionSizeBytes[i];
								auto samples = (unsigned char*)stream->samples[i];
								frameSnapshot.samples.insert(frameSnapshot.samples.end(), samples, samples + samplesSizeInBytes);
							}
						}
					}//if (frameSnapshotEnabled)
