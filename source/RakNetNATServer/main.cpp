...
struct NatPunchthroughServerEx: public NatPunchthroughServer {
	bool ContainsUser(const RakNetGUID &key) const {
		return this->users.HasData(key);
	}
	
	//override
	virtual virtual void OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming)
	{
		if (ContainsUser(rakNetGUID))//already has this user, reject and tell user
		{
			RakNet::BitStream bi;
			bi.Write(static_cast<unsigned char>(ID_USER_PACKET_NAT_SERVER_ERR_GUID_ALREADY_EXIST));
			GetRakPeerInterface()->Send(&bi, HIGH_PRIORITY, RELIABLE, 1, systemAddress, false);
			
			if (this->natPunchthroughServerDebugInterface)
			{
				char log[512];
				char address_str[256];
				char guid_str[50];
				systemAddress.ToString(true, address_str);
				rakNetGUID.ToString(guid_str);
				sprintf(log, "* Rejected %s due to guid %s already exist\n", address_str, guid_str);

				this->natPunchthroughServerDebugInterface->OnServerMessage(log);
			}//if (this->natPunchthroughServerDebugInterface)
			return;
		}//if (ContainsUser(rakNetGUID))
		else {
			//tell user that we accept
			RakNet::BitStream bi;
			bi.Write(static_cast<unsigned char>(ID_USER_PACKET_NAT_SERVER_ACCEPTED));
			GetRakPeerInterface()->Send(&bi, HIGH_PRIORITY, RELIABLE, 1, systemAddress, false);

			NatPunchthroughServer::OnNewConnection(systemAddress, rakNetGUID, isIncoming);
		}
	}
};
...
struct NatPunchthroughServerFramework : public SampleFramework, public NatPunchthroughServerDebugInterface_Printf
{
	...
	virtual void Init(RakNet::RakPeerInterface *rakPeer)
	{
		if (isSupported==SUPPORTED)
		{
			nps = new NatPunchthroughServerEx;
	...
	virtual void ProcessPacket(RakNet::RakPeerInterface *rakPeer, Packet *packet)
	{
		...
		case ID_USER_PACKET_ENUM_CHECK_GUID://client requested to verify if the give guids exist or not
			if (nps)
			{
				RakNet::BitStream bi(packet->data + 1, packet->length - 1, false);

				unsigned int numGuids;//number of guids to check
				bi.Read(numGuids);

				if (numGuids > MAX_GUIDS_TO_CHECK)
					numGuids = MAX_GUIDS_TO_CHECK;

				const size_t guid_size = sizeof(((RakNet::RakNetGUID*)0)->g);
				unsigned char replyBuffer[1 + sizeof(unsigned int) + guid_size * MAX_GUIDS_TO_CHECK];

				//return invalid guids to client
				replyBuffer[0] = ID_USER_PACKET_ENUM_INVALID_GUID_LIST;

				unsigned int invalidGuids = 0;
				unsigned char *bufferPtr = replyBuffer + 1 + sizeof(unsigned int);
				for (unsigned int i = 0; i < numGuids; ++i) {
					RakNet::RakNetGUID guidToCheck;
					bi.Read(guidToCheck);

					if (!nps->ContainsUser(guidToCheck))
					{
						if (!RakNet::BitStream::IsNetworkOrder())
							RakNet::BitStream::ReverseBytes((unsigned char*)&guidToCheck.g, bufferPtr, sizeof(guidToCheck.g));
						else
							memcpy(bufferPtr, &guidToCheck.g, sizeof(guidToCheck.g));
				...
				//write number of invalid guids
				if (!RakNet::BitStream::IsNetworkOrder())
					RakNet::BitStream::ReverseBytes((unsigned char*)&invalidGuids, replyBuffer + 1, sizeof(invalidGuids));
				else
					memcpy(replyBuffer + 1, &invalidGuids, sizeof(invalidGuids));

				//send reply to client
				rakPeer->Send((const char*)replyBuffer, bufferPtr - replyBuffer, HIGH_PRIORITY, RELIABLE, 1, packet->systemAddress, false);
			}//if (nps)
			break;
		}
	}
...
int main(int argc, char **argv)
{
	...
	if (i>=2)
	{
		unsigned int ipIdx1 = 0, ipIdx2 = 0;

		do {
			printf("Choosing 1st IP:");
			scanf("%d", &ipIdx1);
			ipIdx1 --;
		} while(ipIdx1 >= i);

		strcpy(sd[0].hostAddress, ipList[ipIdx1].ToString(false));

		do {
			printf("Choosing 2nd IP:");
			scanf("%d", &ipIdx2);
			ipIdx2 --;
		} while(ipIdx2 >= i && ipIdx2 == ipIdx1);
		sd[1].port=DEFAULT_RAKPEER_PORT+1;
		strcpy(sd[1].hostAddress, ipList[ipIdx2].ToString(false));
		sdLen=2;
	}
