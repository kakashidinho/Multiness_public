...
static Sound::Input *cNstSoundInput;
...
class MyServerDiscoveryDebug : public HQRemote::SocketServerDiscoverClientHandler::DiscoveryDelegate {
public:
	virtual void onNewServerDiscovered(HQRemote::SocketServerDiscoverClientHandler* handler, uint64_t request_id, const char* addr, int reliablePort, int unreliablePort, const char* desc) override {
		HQRemote::Log("Discovered %s %s:(tcp = %d, udp = %d)\n", desc, addr, reliablePort, unreliablePort);
	}
};

class MyServerDiscoveryHandler : public HQRemote::SocketServerDiscoverClientHandler {
public:
	MyServerDiscoveryHandler(HQRemote::SocketServerDiscoverClientHandler::DiscoveryDelegate* delegate)
	: SocketServerDiscoverClientHandler(delegate)
	{}
	
	~MyServerDiscoveryHandler() {
		stop();
	}
};

static MyServerDiscoveryDebug gServerDiscoveryDebug;
static MyServerDiscoveryHandler* gServerDiscoveryHandler = new MyServerDiscoveryHandler(&gServerDiscoveryDebug);
...
static uint NST_CALLBACK SoundReadInput(void* userData, Sound::Input& input, void* samples, uint maxSamples) {
	return audio_read_input(input, samples, maxSamples);
}

static void NST_CALLBACK SoundUpdateSettings(void* userData) {
	Sound nesSound(emulator);
	
	bool nesSoundStero = nesSound.GetSpeaker() == Sound::SPEAKER_STEREO;
	if (nesSound.GetSampleRate() != conf.audio_sample_rate
		|| nesSoundStero != conf.audio_stereo )
	{
		conf.audio_sample_rate = (int)nesSound.GetSampleRate();
		conf.audio_stereo = nesSoundStero;
...
static void NST_CALLBACK MachineEvent(void* userData, Machine::Event event, Nes::Result result) {
	switch (event) {
		case Nes::Api::Machine::EVENT_REMOTE_DISCONNECTED:
			fprintf(stderr, "Fatal: Remote connection timeout or disconnected\n");
...
bool __attribute__((weak)) nst_choose_rom_dialog(std::string& file) {
	return false;
}

bool __attribute__((weak)) nst_app_additional_eventHandler_iterate() {
	return false;
}
...
void nst_play() {
	...
	cNstSoundInput = new Sound::Input;
...
void nst_load(const char *filename) {
	...
	bool isRemote = strstr(filename, REMOTE_PREFIX) == filename;
	...
	if (!isRemote)
	{
		...
		result = machine.LoadRemote(filename + sizeof(REMOTE_PREFIX) - 1, REMOTE_HOST_PORT);
	}
	...
	if (!isRemote && conf.remote_controller_idx != -1)
		machine.EnableRemoteController(conf.remote_controller_idx, REMOTE_HOST_PORT);
...
int main(int raw_argc, char *raw_argv[]) {
	...
	char rom_file_from_dialog[1024];
	...
	if (argc == 1) {
	#endif
		//attempt to prompt for input file
		std::string rom_file;
		if (nst_choose_rom_dialog(rom_file) && rom_file.length() < sizeof(rom_file_from_dialog))
		{
			memcpy(rom_file_from_dialog, rom_file.c_str(), rom_file.length());
			arg_list.push_back(rom_file_from_dialog);
			
			argc = (int)arg_list.size();
			argv = arg_list.data();
		}
		else {
			cli_show_usage();
	...
	Machine::eventCallback.Set(MachineEvent, userData);
	...
	Sound::Output::updateSettingsCallback.Set(SoundUpdateSettings, userData);
		
	Sound::Input::readCallback.Set(SoundReadInput, userData);
	...
	if (argc > 1) {
		...
		//attempt to prompt for correct input file
		std::string rom_file;
		if (!loaded && nst_choose_rom_dialog(rom_file))
		{
	...
	while (!nst_quit) {
		...
		if (playing) {
			...
			if (!nst_app_additional_eventHandler_iterate())
			{
				...
				// Execute a frame
				if (timing_frameskip()) {
					emulator.Execute(NULL, cNstSound, cNstPads, cNstSoundInput);
				}
				else { emulator.Execute(cNstVideo, cNstSound, cNstPads, cNstSoundInput); }
			}
		}
	}
