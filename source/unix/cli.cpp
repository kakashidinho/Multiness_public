...
void cli_show_usage() {
	printf("Usage: nestopia [options] [FILE]|remote:[IP]\n");
	printf("\nOptions:\n");
	...
	printf("  -1                      Enable 1st remote controller\n\n");
	printf("  -2                      Enable 2nd remote controller\n\n");
	printf("  -i                      Enable microphone\n\n");
	printf("  -z                      Disable microphone\n\n");
...
void cli_handle_command(int argc, char *argv[]) {
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "defhl:mnopqrs:tuvw12iz",
		...
		switch(c) {
			...
			case '1':
				conf.remote_controller_idx = 0;
				break;
			case '2':
				conf.remote_controller_idx = 1;
				break;
