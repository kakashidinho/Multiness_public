...
void config_file_write() {
	...
	if (fp != NULL)	{
		...
		fprintf(fp, "remote_controller_idx=%d\n", conf.remote_controller_idx);
		fprintf(fp, "mic_enable=%d\n", conf.mic_enable);
...
static int config_match(void* user, const char* section, const char* name, const char* value) {
	...
	else if (MATCH("misc", "remote_controller_idx")) { pconfig->remote_controller_idx = atoi(value); }
	else if (MATCH("misc", "mic_enable")) { pconfig->mic_enable = atoi(value); }
