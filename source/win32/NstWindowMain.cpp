	...
	namespace Window
	{
					...
					if (emulator.Resume())
					{
						if (emulator.IsGame())
						{
							for (uint skips=frameClock.GameSynchronize( video.ThrottleRequired(frameClock.GetRefreshRate()) ); skips; --skips)
								emulator.Execute( NULL, sound.GetOutput(), input.GetOutput(), sound.GetInput() );

							emulator.Execute( video.GetOutput(), sound.GetOutput(), input.GetOutput(), sound.GetInput());
							video.PresentScreen();
							input.Poll();
						}
						else
						{
							...
							emulator.Execute( NULL, sound.GetOutput(), NULL, sound.GetInput());
		...
		void Main::OnEmuEvent(const Managers::Emulator::Event event,const Managers::Emulator::Data data)
		{
			switch (event)
			{
				...
				case Managers::Emulator::EVENT_REMOTE_CONTROLLER_ENABLED:
				case Managers::Emulator::EVENT_REMOTE_CONTROLLER_DISABLED:
				case Managers::Emulator::EVENT_CLIENT_CONNECTED:
				case Managers::Emulator::EVENT_CLIENT_DISCONNECTED:
				{
					//game title
					Path name;
					...
					case Managers::Emulator::EVENT_CLIENT_CONNECTED:
					{
						auto info = (const char*)data;
						ss << " (" << info << " connected)";
					}
						break;
					default:
					{
						auto connHandler = emulator.GetRemoteControllerHostConnHandler();
						auto raknetHandler = dynamic_cast<Nes::Remote::ConnectionHandlerRakNet*>(connHandler.get());
						if (raknetHandler)
						{
							ss << " (Remote enabled, guid=" << raknetHandler->getGUID().g << ")";
						}
					}
						break;
					}//switch (event)

					window.Text() << ss.str().c_str();

					break;
				}

				case Managers::Emulator::EVENT_LOAD_REMOTE:
					window.Text() << "Remote control connecting ...";
					break;
				case Managers::Emulator::EVENT_REMOTE_CONNECTED:
					window.Text() << "Remote controlling (0 KB/s)...";
					break;
				case Managers::Emulator::EVENT_REMOTE_DATA_RATE:
				{
					float rate = *(float*)data;
					rate /= 1024.f;
					std::stringstream ss;
					ss << "Remote controlling (" << rate << " KB/s)...";
