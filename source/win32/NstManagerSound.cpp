...
namespace Nestopia
{
	namespace Managers
	{
		struct Sound::Callbacks
		{
			...
			static void NST_CALLBACK UpdateSettings(Nes::Sound::UserData);//LHQ

			static uint NST_CALLBACK Read(Nes::Sound::UserData, Nes::Sound::Input& input, void* samples, uint maxSamples);//LHQ: sound input callback
		};
		...
		recorder    ( new Recorder(m,dialog->GetRecorder(),e) )
		{
			...
			Nes::Sound::Output::updateSettingsCallback.Set(&Callbacks::UpdateSettings, this);//LHQ
			
			Nes::Sound::Input::updateSettingsCallback.Set(&Callbacks::UpdateSettings, this);//LHQ
			Nes::Sound::Input::readCallback.Set(&Callbacks::Read, this);//LHQ
		...
		Sound::~Sound()
		{
			...
			Nes::Sound::Output::updateSettingsCallback.Unset();

			Nes::Sound::Input::updateSettingsCallback.Unset();//LHQ
			Nes::Sound::Input::readCallback.Unset();//LHQ
		}
		...
		//LHQ
		void NST_CALLBACK Sound::Callbacks::UpdateSettings(Nes::Sound::UserData data) {
			Sound& sound = *static_cast<Sound*>(data);

			sound.UpdateSettings();
		}

		uint NST_CALLBACK Sound::Callbacks::Read(Nes::Sound::UserData data, Nes::Sound::Input& input, void* samples, uint maxSamples)
		{
			if (&input == NULL)
				return 0;
			Sound& sound = *static_cast<Sound*>(data);
			return sound.directSound.ReadInput(samples, maxSamples);
		}
		//end LHQ
