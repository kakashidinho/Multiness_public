...
namespace Nestopia
{
	namespace Managers
	{
		...
		clipboard    ( e, m )
		{
			//LHQ: map between menu item resource id to controller index
			remoteMenuItemId2ControllerIds.insert(std::pair<uint, uint>(IDM_ENABLEREMOTECONTROLLER_PORT1, 0));
			remoteMenuItemId2ControllerIds.insert(std::pair<uint, uint>(IDM_ENABLEREMOTECONTROLLER_PORT2, 1));

			remotePort = cfg.GetRemotePort();
			natServerAddr = cfg.GetNATServerAddress();
			natServerPort = cfg.GetNATServerPort();

			memset(abPressedState, 0, sizeof(abPressedState));
			//end LHQ
			...
			static const Window::Menu::CmdHandler::Entry<Input> commands[] =
			{
				...
				{ IDM_ENABLEREMOTECONTROLLER_PORT1,			 &Input::OnCmdToggleRemoteController1 },//LHQ
				{ IDM_ENABLEREMOTECONTROLLER_PORT2,			 &Input::OnCmdToggleRemoteController2 },//LHQ
			};
		...
		void Input::StopEmulation()
		{
			...
			memset(abPressedState, 0, sizeof(abPressedState));//LHQ
		}
		...
		void Input::OnEmuEvent(const Emulator::Event event,Emulator::Data data)
		{
			...
			switch (event)
			{
				...
				case Emulator::EVENT_LOAD_REMOTE:
				case Emulator::EVENT_UNLOAD:
					...
					break;
				case Emulator::EVENT_REMOTE_CONTROLLER_ENABLED:
				case Emulator::EVENT_REMOTE_CONTROLLER_DISABLED:
				{
					auto enable = event == Emulator::EVENT_REMOTE_CONTROLLER_ENABLED;
					switch (data) {
					case 0:
						menu[IDM_ENABLEREMOTECONTROLLER_PORT1].Check(enable);
						break;
					case 1:
						menu[IDM_ENABLEREMOTECONTROLLER_PORT2].Check(enable);
						break;
					}
				}
					break;
		...
		//LHQ
		void Input::OnCmdToggleRemoteController1(uint params) {
			OnCmdToggleRemoteController(IDM_ENABLEREMOTECONTROLLER_PORT1, params);
		}

		void Input::OnCmdToggleRemoteController2(uint params) {
			OnCmdToggleRemoteController(IDM_ENABLEREMOTECONTROLLER_PORT2, params);
		}

		bool Input::OnCmdToggleRemoteController(uint id, uint params) {
			bool prevChecked = menu[id].Checked();
			
			//TODO: for now we only allow one controller to be remote controlled
			menu[IDM_ENABLEREMOTECONTROLLER_PORT1].Check(false);
			menu[IDM_ENABLEREMOTECONTROLLER_PORT2].Check(false);

			emulator.DisableRemoteController(0);
			emulator.DisableRemoteController(1);

			menu[id].Check(!prevChecked);

			if (!prevChecked)
			{
				auto modeParams = Window::EnableRemoteControl().Open();

				auto idx = remoteMenuItemId2ControllerIds[id];

				switch (modeParams.mode)
				{
				case Window::EnableRemoteControl::REMOTE_CTL_MODE_LAN:
					emulator.EnableRemoteController(idx, remotePort);
					break;
				case Window::EnableRemoteControl::REMOTE_CTL_MODE_INTERNET:
				{
					char guid_str[128];
					RakNet::RakNetGUID guid(modeParams.internetParams.guid);
		...
		bool NST_CALLBACK Input::Callbacks::PollPad(UserData data,Pad& pad,uint index)
		{
			...
			//LHQ combon button
			uint abPressedState = (keys[Settings::PAD_KEY_AB].GetState());
			//end LHQ
			...
			if (input.autoFire)
			{
				...
				if (!abPressedState)
				{
					abPressedState = (keys[Settings::PAD_KEY_AUTOFIRE_AB].GetState());
				}
			}
			//LHQ combon button
			if (abPressedState != input.abPressedState[index])
			{
				if (abPressedState)
					buttons |= Pad::A;
				else
			...
			else if (abPressedState)
				buttons |= Pad::A | Pad::B;

			input.abPressedState[index] = abPressedState;
			//end LHQ
