...
namespace Nestopia
{
	namespace Managers
	{
		...
		doFullscreen ( cfg["netplay"]["fullscreen"].Yes() )
		{
			...
			static const Window::Menu::CmdHandler::Entry<Netplay> remoteCommands[] =
			{
				{ IDM_NETPLAY_REMOTECONTROL_LAN,				&Netplay::OnCmdLANRemoteCtl},
				{ IDM_NETPLAY_REMOTECONTROL_INTERNET,			&Netplay::OnCmdInternetRemoteCtl },
				{ IDM_NETPLAY_ENABLELOWRESOLUTIONREMOTE,		&Netplay::OnCmdLowResRemoteCtl },
			};

			menu.Commands().Add(this, remoteCommands);
			//end LHQ
			const Dll dll;
		...
		void Netplay::OnEmuEvent(const Emulator::Event event,const Emulator::Data data)
		{
			switch (event)
			{
				...
				case Emulator::EVENT_UNLOAD:
					...
					break;
				//LHQ
				case Emulator::EVENT_REMOTE_LOWRES:
					menu[IDM_NETPLAY_ENABLELOWRESOLUTIONREMOTE].Check(data);
					break;
				//end LHQ
			}
		}
		...
		//LHQ
		void Netplay::OnCmdLANRemoteCtl(uint param) {
			Window::RemoteControl remoteOpenDlg;
			bool cancelled = false;
			const auto& remoteAddr = remoteOpenDlg.Open(cancelled);
			
			if (!cancelled)
				emulator.LoadRemote(remoteAddr, this->remotePort);
		}

		void Netplay::OnCmdInternetRemoteCtl(uint param) {
			Window::InternetRemoteControl remoteOpenDlg;
			auto params = remoteOpenDlg.Open();

			if (!params.valid)
				return;

			RakNet::RakNetGUID remoteGUID(params.remote_guid);
			...
			char myGUID_str[128];
			connHandler->getGUID().ToString(myGUID_str);

			emulator.LoadRemote(connHandler, myGUID_str);
		}

		void Netplay::OnCmdLowResRemoteCtl(uint) {
			auto control = menu[IDM_NETPLAY_ENABLELOWRESOLUTIONREMOTE];
			control.Check(!control.Checked());

			emulator.EnableLowResRemoteControl(control.Checked());
		}
	}
}
