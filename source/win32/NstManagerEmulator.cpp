...
namespace Nestopia
{
	namespace Managers
	{
			...
			static void NST_CALLBACK OnMachine(Nes::User::UserData user,Nes::Machine::Event event,Nes::Result result)
			{
				...
				switch (event)
				{
					case Nes::Machine::EVENT_LOAD:
						...
						break;
					//LHQ
					case Nes::Machine::EVENT_LOAD_REMOTE:
						if (NES_FAILED(result))
						{
							...
							emulator.events(EVENT_LOAD_REMOTE);
						break;
					case Nes::Machine::EVENT_REMOTE_CONTROLLER_ENABLED:
						emulator.events(EVENT_REMOTE_CONTROLLER_ENABLED, result);
						break;
					case Nes::Machine::EVENT_REMOTE_CONTROLLER_DISABLED:
						emulator.events(EVENT_REMOTE_CONTROLLER_DISABLED, result);
						break;
					case Nes::Machine::EVENT_REMOTE_CONNECTION_INTERNAL_ERROR:
					{
						emulator.events(EVENT_REMOTE_CONNECTION_INTERNAL_ERROR, result);

						//result is address of error message string
						auto errMsg = (const char*)result;

						std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
						std::wstring errMsgWide = converter.from_bytes(errMsg);

						Window::User::Fail(errMsgWide.c_str());
					}
						break;
					case Nes::Machine::EVENT_REMOTE_CONNECTED: 
						emulator.events(EVENT_REMOTE_CONNECTED);
						break;
					case Nes::Machine::EVENT_REMOTE_DISCONNECTED:
						Window::User::Fail(L"Remote connection timeout or disconnected");//TODO: localized text
						break;
					case Nes::Machine::EVENT_CLIENT_CONNECTED:
						emulator.events(EVENT_CLIENT_CONNECTED, result);
						break;
					case Nes::Machine::EVENT_CLIENT_DISCONNECTED:
						emulator.events(EVENT_CLIENT_DISCONNECTED, result);
						break;
					case Nes::Machine::EVENT_REMOTE_LOWRES:
						emulator.events(EVENT_REMOTE_LOWRES, NES_SUCCEEDED(result));
						break;
					case Nes::Machine::EVENT_REMOTE_DATA_RATE:
						emulator.events(EVENT_REMOTE_DATA_RATE, result);
						break;
					//end LHQ
		...
		(
			...
			Nes::Sound::Input* const inputSound
		)
		{
			if (state.running)
			{
				...
				Nes::Emulator::Execute( video, sound, input, inputSound );
			}
		}
		...
		bool Emulator::LoadRemote(const String::Heap<char>& remoteIp, int remotePort) {
			auto re = Nes::Machine(*this).LoadRemote(remoteIp.Ptr(), remotePort);
			return NES_SUCCEEDED(re);
		}

		bool Emulator::LoadRemote(std::shared_ptr<HQRemote::IConnectionHandler> connHandler, const char* myName)
		{
			auto re = Nes::Machine(*this).LoadRemote(connHandler, myName);
			return NES_SUCCEEDED(re);
		}

		bool Emulator::EnableRemoteController(uint idx, int networkListenPort) {
			char hostName[MAX_COMPUTERNAME_LENGTH + 1] = {0};
			DWORD hostNameMaxLen = MAX_COMPUTERNAME_LENGTH;
			GetComputerNameA(hostName, &hostNameMaxLen);

			auto re = Nes::Machine(*this).EnableRemoteController(idx, networkListenPort, hostName);
			return NES_SUCCEEDED(re);
		}

		bool Emulator::EnableRemoteController(uint idx, std::shared_ptr<HQRemote::IConnectionHandler> connHandler, const char* myName)
		{
			auto re = Nes::Machine(*this).EnableRemoteController(idx, connHandler, myName);
			return NES_SUCCEEDED(re);
		}

		//get connection handler which was passed to EnableRemoteController()
		std::shared_ptr<HQRemote::IConnectionHandler> Emulator::GetRemoteControllerHostConnHandler()
		{
			return Nes::Machine(*this).GetRemoteControllerHostConnHandler();
		}

		void Emulator::DisableRemoteController(uint idx)
		{
			Nes::Machine(*this).DisableRemoteController(idx);
		}

		void Emulator::EnableLowResRemoteControl(bool enable) {
			Nes::Machine(*this).EnableLowResRemoteControl(enable);
		}
		...
		uint Emulator::ResultToString(const Nes::Result result)
		{
			...
			else
			{
				switch (result)
				{
					case Nes::RESULT_ERR_CONNECTION:			   return IDS_EMU_ERR_CONNECTION;//LHQ
